...
for i in range(0, 81):
    gamma.append(float(i) / 20.0)
    eta_saddle.append(1.0 / sqrt(2.0 * gamma[i] + 1.0))

for i in range(0, len(eta_i)):
    eta_i[i] = float(i) / len(eta_i)
    eta_j[i] = float(i) / len(eta_j)
...
def animate(k):
    ...
    free_energy = array((eta_i**4.0 / 4.0) - (eta_i**2.0 / 2.0) + (eta_j**4.0 / 4.0) - (eta_j**2.0 / 2.0) + (gamma[k] * eta_i**2.0 * eta_j**2.0) + 1.0 / 4.0)
    free_energy = free_energy / linalg.norm(free_energy) # FIXME: Figure out how to normalize the data.
    cont = contourf(eta_i, eta_j, free_energy, 40)
...
ani = FuncAnimation(fig, animate, frames = len(gamma), interval=150)
if saveMovie:
